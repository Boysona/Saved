...
import asyncio
# import speech_recognition as sr # REMOVED: No longer needed with faster-whisper
...
WEBHOOK_URL = "https://media-transcriber-bot-sal5.onrender.com"
...
TTS_VOICES_BY_LANGUAGE = {
    ...
    "French 🇫🇷": [
        ...
        "fr-CH-ArianeNeural", "fr-CH-FabriceNeural", "fr-CH-CharlineNeural", "fr-BE-CamilleNeural" # Fixed typo fr-BE-CharlineNeural
    ],
    "German 🇩🇪": [
...
LANGUAGES = [
    ...
    {"name": "Filipino", "flag": "🇵🇭", "code": "tl"}, # Filipino code is 'tl' for Whisper
...
def handle_text_for_tts_or_fallback(message):
    ...
    if user_tts_mode.get(uid): # If user is in TTS input mode
        # Use asyncio.run to call the async function from a sync context if needed
        # Or, ideally, ensure bot.infinity_polling runs an event loop.
        # For simplicity with telebot's threaded mode, we'll start a new thread.
        threading.Thread(target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, uid, message.text))).start()
        # After processing, you might want to turn off TTS mode if it's a one-off text conversion
        # user_tts_mode[uid] = False # Uncomment this if you want single TTS conversion per text message
    else:
        # Fallback for other non-media messages
        bot.send_message(message.chat.id, "I only transcribe voice messages, audio, or video. To convert text to speech, use the /text_to_speech command first.")
