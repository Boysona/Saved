...
# --- BOT CONFIGURATION ---
TOKEN = "7790991731:AAGiX824l_3PbGS5ZiQTuHIeRQX71JLTa3o" # Both bots used the same token, so it's unified
ADMIN_ID = 5978150981
...
# --- In-memory caches ---
local_user_data = {}            # { user_id: { "last_active": "...", "tts_count": N, "stt_count": N, ... } }
...
def is_user_subscribed(user_id):
    ...
    Returns True for group chats (negative user_id).
    """
    # For group chats, always return True (no subscription required)
    if isinstance(user_id, int) and user_id < 0:  # Negative user_id indicates a group chat
        return True
...
def subscription_required(func):
    ...
    Only checks in private chats - group chats can always use the bot.
    """
    def wrapper(message):
        # Skip subscription check for group chats
...
def start_handler(message):
    ...
    if user_id == ADMIN_ID:
        ...
        # For group chats, send a simpler message
        if message.chat.type != 'private':
            bot.send_message(
                message.chat.id,
                "👋 Hi! I'm a transcription and text-to-speech bot. "
                "You can use my services directly in this group - no subscription needed!"
            )
            return
...
def status_handler(message):
    ...
    active_month = users_collection.count_documents({
        "last_active": {
    ...
    # Calculate total processing time (assuming average 5 seconds per TTS and 30 seconds per STT)
    total_processing_seconds = (total_tts * 5) + (total_stt * 30)
    processing_hours, rem = divmod(total_processing_seconds, 3600)
    processing_minutes = rem // 60
    ...
    text = (
        ...
        "👥 Users:\n"
        f"▫️ Total registered users: {total_users}\n"
        f"▫️ Active this month: {active_month}\n"
        f"▫️ Active this week: {active_week}\n"
        f"▫️ Used today: {active_today}\n\n"
        "🎧 Processing Data:\n"
        f"▫️ Files processed for transcription: {total_stt}\n"
        f"▫️ Text-to-audio conversions: {total_tts}\n"
        f"▫️ Total processing time: {processing_hours} hours and {processing_minutes} minutes\n\n"
        "⸻\n\n"
        "🙏 Thank you for using our service!\n"
        "If you encounter any issues or need assistance, please contact us: @user33230"
      )
...
def build_stt_language_keyboard():
    # Set row width to 3 for STT languages
    row_width = 3
...
def send_stt_language_prompt(message):
    ...
    bot.send_message(
        message.chat.id,
        f"📝 Choose your Media (Voice, Audio, Video) file language for transcription using the below buttons:\n"
...
def set_bot_commands():
    commands = [
        ...
        BotCommand("set_stt_language", "Select language for transcription "), # New command for STT language
