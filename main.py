...
def button_translate_handler(call):
    ...
    if preferred_lang:
        ...
        do_translate_with_saved_lang(call.message, uid, preferred_lang)
    else:
...
def button_summarize_handler(call):
    ...
    if preferred_lang:
        ...
        do_summarize_with_saved_lang(call.message, uid, preferred_lang)
    else:
...
def callback_translate_to(call):
    ...
    do_translate_with_saved_lang(call.message, uid, lang)
    bot.answer_callback_query(call.id)
...
def callback_summarize_in(call):
    ...
    do_summarize_with_saved_lang(call.message, uid, lang)
    bot.answer_callback_query(call.id)

# Modified do_translate to use a provided language
def do_translate_with_saved_lang(message, uid, lang):
...
# Modified do_summarize to use a provided language
def do_summarize_with_saved_lang(message, uid, lang):
    original = last_transcription.get(uid, "")
    if not original:
...
def handle_translate(message):
    ...
    preferred_lang = user_language_settings.get(uid)
    if preferred_lang:
        do_translate_with_saved_lang(message, uid, preferred_lang)
    else:
        markup = generate_language_keyboard("translate_to")
        bot.send_message(
            message.chat.id,
...
def handle_summarize(message):
    ...
    preferred_lang = user_language_settings.get(uid)
    if preferred_lang:
        do_summarize_with_saved_lang(message, uid, preferred_lang)
    else:
        markup = generate_language_keyboard("summarize_in")
        bot.send_message(
            message.chat.id,
...
if __name__ == "__main__":
    ...
    set_bot_info()
    bot.delete_webhook()
    bot.set_webhook(url="https://only-me-cwkd.onrender.com")
